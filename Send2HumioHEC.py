...
class Send_to_HEC():
    ...
    def send_to_HEC(event_data):
        ...
        try:
            ...
            r = requests.post(url=HumioHECurl, headers=header, data=event_data.encode(
                'utf-8'), verify=config.HumioHECVerify_IntelIndicators, timeout=300)
            transmit_result = r.status_code
            logging.info('IntelIndicators2Humio v' + version +
                         ' HEC: Transmission status code for data push to HEC= ' + str(transmit_result))
            logging.info('IntelIndicators2Humio v' + version + ' HEC: Transmission results for data push to HEC= ' + str(r.json))

        except requests.exceptions.RequestException as e:
            error = str(e)
            logging.info('IntelIndicators2Humio v' + version +
                         ' HEC: Unable to evaluate and transmit sensor_data event: Error: ' + error)
            try:
