...
class CS_Intel_Indicators_Humio():
    def get_cs_intel(self):
        ...
        falcon = APIHarness(client_id=config.CS_indicators_client_id,
                            client_secret=config.CS_indicators_client_secret, base_url=config.CS_indicators_base_url)
        ...
        with open('CS_Intel_to_Humio_Tracker', 'a+') as t:
            ...
            if len(read) == 0:
                ...
                logging.info('IntelIndicators2Humio v' + version +
                             ': This is last marker collected per the tracker: ' + tracker[-1])
        ...
        logging.info('IntelIndicators2Humio v' + version +
                     ': Connection to Intel API resulted with status code: ' + str(status_code))
        ...
        if status_code.startswith('20'):
            ...
            while pagination == True:
                ...
                try:
                    ...
                    logging.info('IntelIndicators2Humio v' + version +
                                 ': Connection to OAuth2 Token API resulted with status code: ' + str(status_code))

                except Exception as e:
                    logging.error('IntelIndicators2Humio v' + version +
                                  ': Unable to obtain OAuth2 token from CrowdStrike' + e.message + '  ' + e.args)
                    sys.exit('IntelIndicators2Humio v' + version +
                             ' : Unable to obtain OAuth2 token to complete data collection, please correct any issues and try again')
                ...
                try:
                    ...
                    logging.error('IntelIndicators2Humio v' + version +
                                  ': Unable to obtain OAuth2 token from CrowdStrike' + e.message + '  ' + e.args)
                    sys.exit('IntelIndicators2Humio v' + version +
                             ' : Unable to connect to next collection of indicators, please correct any issues and try again')
                ...
                last_ind = indicators[-1]
                logging.info('IntelIndicators2Humio v' + version + ': Recording the last marker value to tracker: last updated=' +
                             (str(last_ind['last_updated'])) + '    _marker value= ' + str(last_ind['_marker']))
                with open('CS_Intel_to_Humio_Tracker', 'a+') as t:
                    t.write('\n'+last_ind['_marker'])
